...
// Kiểm tra trống
function Kiemtra_Trong(cap, idTruong, idKyThi) {
    Reset_Kiemtra();
    var trangthai = true;
    if (cap == 4) {
        if (idTruong == 0) {
...
function DongY_ChonKyThi() {
    ...
    // Kiểm tra trống
    var kiemtra = Kiemtra_Trong(cap, idTruong, idKyThi);
    if (kiemtra) {
        $('#mdlChonKyThi').modal('hide');
        window.location = url + '?idKyThi=' + idKyThi + '&idTruong=' + idTruong + '&idDoiTuong=' + idDoiTuong;
    }
};
